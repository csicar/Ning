...
class LowLevelMDnsScanner(
    ...
    suspend fun probeMDns(serviceName: String) =
        withContext(Dispatchers.IO) {
    ...
    private fun ByteArray.rangeToInt(
        start: Int,
        end: Int,
    ): Int {
        var value = 0
        ...
        val dnsAnswer =
            when (parseRecordType(type)) {
                ...
                RecordType.A ->
                    DnsAnswer(
                        combinedName,
                        listOf(Inet4Address.getByAddress(byteArray.copyOfRange(dataIndex, dataIndex + 4)).toString()),
                        "",
                    )
                RecordType.AAAA ->
                    DnsAnswer(
                        combinedName,
                        listOf(Inet6Address.getByAddress(byteArray.copyOfRange(dataIndex, dataIndex + 16)).toString()),
                        "",
                    )
                else -> {
    ...
    fun parseRecordType(typeCode: Int) =
        when (typeCode) {
            1 -> RecordType.A
            2 -> RecordType.NS
            3 -> RecordType.MD
    ...
    data class ScanResult(
        val content: List<DnsAnswer>,
        val ip: InetAddress,
        val port: Int,
    ) {
        /**
         * Extract the human-readable instance name from the first PTR or SRV answer.
         * domainName contains parts like ["My Device", "_ssh", "_tcp", "local"],
         * where the first element that doesn't start with "_" is the instance name.
         */
        val hostname: String?
            get() =
                content
                    .firstOrNull { it.domainName.isNotEmpty() }
                    ?.domainName
