...
data class ArpEntry(
    ...
    companion object {
        fun from(
            ip: String,
            mac: String,
        ) = ArpEntry(InetAddress.getByName(ip), MacAddress(mac))
    }
}
    ...
    suspend fun getFromAllSources() =
        withContext(Dispatchers.Default) {
            listOf(
                ...
                .filter { it.hwAddress.address != "FAILED" }
                .associateBy { it.ip }
        }

    private suspend fun getArpTableFromFile(): Sequence<ArpEntry> =
